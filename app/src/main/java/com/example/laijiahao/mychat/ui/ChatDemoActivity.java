...
public class ChatDemoActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!EMClient.getInstance().isLoggedInBefore()) {
			Intent intent = new Intent(ChatDemoActivity.this, LoginActivity.class);
			...
			public void onClick(View v) {
				...
				if (!TextUtils.isEmpty(chatId)) {
					...
					if (chatId.equals(currUsername)) {
						Toast.makeText(ChatDemoActivity.this, "不能和自己聊天", Toast.LENGTH_SHORT).show();
						return;
					}
					// 跳转到聊天界面，开始聊天
					Intent intent = new Intent(ChatDemoActivity.this, ChatActivity.class);
					// EaseUI封装的聊天界面需要这两个参数，聊天者的username，以及聊天类型，单聊还是群聊
					intent.putExtra("userId", chatId);
					intent.putExtra("chatType", EMMessage.ChatType.Chat);
					startActivity(intent);
				} else {
					Toast.makeText(ChatDemoActivity.this, "Username 不能为空", Toast.LENGTH_LONG).show();
				}
			}
		});
